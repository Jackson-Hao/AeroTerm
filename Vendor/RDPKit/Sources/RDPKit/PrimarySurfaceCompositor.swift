...
final class RDPPrimarySurfaceCompositor {
    ...
    private var lastDirtyBatchStartNs: UInt64 = 0
    private let remoteFX = RDPRemoteFXDecoder()
    private static let maxBatchIntervalNs: UInt64 = 33_000_000
    ...
    var hasPendingDirty: Bool {
        !dirtyRects.isEmpty
    }

    func shouldFlushMaxWait() -> Bool {
        guard !dirtyRects.isEmpty, lastDirtyBatchStartNs != 0 else {
            return false
        }
        return DispatchTime.now().uptimeNanoseconds &- lastDirtyBatchStartNs >= Self.maxBatchIntervalNs
    }

    func flushPendingFrame() -> RDPGraphicsFrameSnapshot? {
        makeFrame(frameID: nil)
    }
    ...
    private func copySourceRow(
        ...
        for column in 0 ..< sourceWidth {
            ...
            case 15, 16:
                ...
                let greenMask = (1 << greenBits) - 1
                let green = Int((pixel >> 5) & UInt16(greenMask))
                let red = Int((pixel >> (5 + greenBits)) & 0x001F)
                destination[destinationOffset] = UInt8((blue * 255 + 15) / 31)
                destination[destinationOffset + 1] = UInt8((green * 255 + greenMask / 2) / greenMask)
    ...
    private func addDirtyRect(_ rect: RDPFrameRect) {
        if dirtyRects.isEmpty {
            lastDirtyBatchStartNs = DispatchTime.now().uptimeNanoseconds
        }
