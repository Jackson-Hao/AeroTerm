...
struct RDPServerLicensePDU: Equatable, Sendable {
    ...
    static func parseIfPresent(fromTPKT packet: Data) throws -> RDPServerLicensePDU? {
        ...
        case 0xFF:
            ...
            // xrdp 0.9.x writes an arbitrary blob type (0x1428) and zero length.
            if blobLength > 0, blobType != RDPLicenseBinaryBlobType.error {
                throw RDPDecodeError.invalidLicensePDU
            }
...
struct RDPClientLicenseErrorAlertPDU: Equatable, Sendable {
    var channelID: UInt16
    var errorCode: UInt32
    var stateTransition: UInt32

    static func validClient(channelID: UInt16) -> Self {
        Self(
            channelID: channelID,
            errorCode: RDPLicenseErrorCode.validClient,
            stateTransition: RDPLicenseStateTransition.noTransition
        )
    }
    ...
    func encodedUserData() throws -> Data {
        var body = Data()
        body.appendLittleEndianUInt32(errorCode)
        body.appendLittleEndianUInt32(stateTransition)
        try appendLicenseBlob(type: RDPLicenseBinaryBlobType.error, payload: Data(), to: &body)
        return try licenseUserData(messageType: 0xFF, body: body)
    }
}
