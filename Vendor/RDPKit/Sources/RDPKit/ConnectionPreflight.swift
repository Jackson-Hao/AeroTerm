...
private struct MCSConnectionSequence {
    ...
    var sequenceSucceeded: Bool {
        ...
        if graphicsAttempted {
            let hasClassicFrames = firstGraphicsFrame != nil
                || !graphicsFrames.isEmpty
                || !fastPathUpdateMessages.isEmpty
            if graphicsCapsConfirm == nil {
                return graphicsError == nil && hasClassicFrames
            }
    ...
    var failureMessage: String? {
        ...
        if graphicsAttempted {
            ...
            if graphicsUpdatesAttempted {
                guard !graphicsUpdateMessages.isEmpty || hasClassicFrames else {
                    return graphicsError ?? "server graphics update did not contain RDPGFX messages"
                }
                guard !graphicsFrameAcknowledgeData.isEmpty || hasClassicFrames else {
    ...
    do {
        ...
        if licenseResponse?.typeName == "license-request" {
            do {
                ...
                let serverCertificatePublicKey = licenseResponse?.serverCertificatePublicKey
                    ?? connectResponse.serverCertificatePublicKey
                if let storedClientLicense = configuration.storedClientLicense,
                   ...
                   let serverCertificatePublicKey,
                   let serverRandom = licenseRequest.serverRandom {
                    ...
                    // xrdp and many unlicensed servers do not implement a license
                    // server. STATUS_VALID_CLIENT is the standard skip used by
                    // FreeRDP / mstsc against those hosts.
                    requestData = try RDPClientLicenseErrorAlertPDU
                        .validClient(channelID: ioChannelID)
                        .encodedTPKT(userChannelID: userChannelID)
                    timeoutDescription = "RDP Client License Error Alert Response"
                }
            ...
            if let licenseFollowup = try? RDPServerLicensePDU.parseIfPresent(fromTPKT: postLicensingResponseData),
               licenseFollowup.typeName == "license-error-valid-client"
                || licenseFollowup.typeName.hasPrefix("license-error-") {
                do {
                    postLicensingResponseData = try receiveApplicationTPKT(
                        on: channel,
                        reader: tpktReader,
                        timeoutSeconds: timeoutSeconds,
                        timeoutDescription: "RDP Demand Active after licensing"
                    )
                } catch {
                    return MCSConnectionSequence(
                        connectInitial: connectInitial,
...
private func performRDPGraphicsDynamicChannelHandshake(
    ...
    do {
        for _ in 0 ..< 16 {
            ...
            guard let staticPDU = try? RDPStaticVirtualChannelPDU.parseIfPresent(
                ...
                var classicResult = result
                var fragmentReassembler = RDPFastPathOutputFragmentReassembler(
                    maximumBufferedByteCount: maximumFastPathFragmentByteCount
                )
                let primarySurfaceCompositor = RDPPrimarySurfaceCompositor(
                    width: desktopWidth,
                    height: desktopHeight
                )
                let remotePointerState = RDPRemotePointerState()
                let targetFrameCount = frameCaptureLimit.map { max(1, $0) }
                var handledClassicGraphics = try recordFastPathOutputPacketIfPresent(
                    packet,
                ...
                if !handledClassicGraphics {
                    handledClassicGraphics = try recordSlowPathGraphicsUpdatePacketIfPresent(
                        packet,
                        result: &classicResult,
                        targetFrameCount: targetFrameCount,
                        ioChannelID: ioChannelID,
                ...
                if handledClassicGraphics {
                    ...
                    try receiveRDPGraphicsUpdateBatch(
                        ...
                        graphicsCapabilitySet: .version8(flags: RDPGFXCapabilityFlags.defaultVersion8),
                        existingPrimarySurfaceCompositor: primarySurfaceCompositor,
                        existingRemotePointerState: remotePointerState,
                        result: &result
                    )
                    return result
                }
    ...
    if graphicsFragment != nil {
        result.error = "server did not complete fragmented RDPGFX capabilities data"
        return result
    }

    // xrdp and other bitmap-only servers never open RDPGFX. Stay in session
    // and decode classic slow-path / fast-path updates instead of failing.
    result.error = nil
    do {
        try receiveRDPGraphicsUpdateBatch(
            shareID: shareID,
        ...
        if result.firstGraphicsFrame == nil, result.graphicsFrames.isEmpty {
            result.error = String(describing: error)
        }
    }
    return result
...
private func receiveRDPGraphicsUpdateBatch(
    ...
    existingPrimarySurfaceCompositor: RDPPrimarySurfaceCompositor? = nil,
    existingRemotePointerState: RDPRemotePointerState? = nil,
    result: inout RDPGraphicsDynamicChannelResult
) throws {
    ...
    let primarySurfaceCompositor = existingPrimarySurfaceCompositor
        ?? RDPPrimarySurfaceCompositor(width: desktopWidth, height: desktopHeight)
    let remotePointerState = existingRemotePointerState ?? RDPRemotePointerState()
    let targetFrameCount = frameCaptureLimit.map { max(1, $0) }
    ...
    func flushPendingClassicFrame(force: Bool) throws {
        guard force || primarySurfaceCompositor.shouldFlushMaxWait() else {
            return
        }
        guard let frame = primarySurfaceCompositor.flushPendingFrame() else {
            return
        }
        appendGraphicsFrame(
            frame,
            to: &result,
            maximumRecordedFrames: recordedGraphicsFrameLimit(targetFrameCount: targetFrameCount)
        )
    ...
    while packetLimit.map({ packetsRead < $0 }) ?? true {
        try throwIfCancelled(shouldCancel, cancellation: cancellation)
        packetsRead += 1
        let useQuietFlush = targetFrameCount == nil
        let packetTimeoutSeconds = useQuietFlush ? nil : timeoutSeconds
        let packet: Data
        do {
            packet = try receiveApplicationTPKT(
                on: channel,
                ...
                shouldCancel: shouldCancel,
                mapRemoteDisconnectToReceiveFailure: false,
                timeout: useQuietFlush ? .milliseconds(20) : nil,
                softTimeout: useQuietFlush
            )
        } catch RDPApplicationReceiveError.softTimeout {
            try flushPendingClassicFrame(force: true)
        ...
        guard let staticPDU = try? RDPStaticVirtualChannelPDU.parseIfPresent(
            ...
            if (try? recordFastPathOutputPacketIfPresent(
                ...
                try flushPendingClassicFrame(force: targetFrameCount != nil)
                if let targetFrameCount,
                   result.graphicsFrames.count >= targetFrameCount
                {
                    return
...
private func waitForApplicationTPKT(
    ...
    do {
        let timeoutAmount = timeout ?? timeoutSeconds.map { TimeAmount.seconds(Int64($0)) }
        guard let timeoutAmount else {
            return try response.wait()
        }
        let timeoutTask = channel.eventLoop.scheduleTask(in: timeoutAmount) {
            if softTimeout {
                ...
                reader.failPendingSoftTimeout(RDPApplicationReceiveError.softTimeout)
            } else {
                reader.fail(RDPPreflightError.receive("\(timeoutDescription) timed out"))
                channel.close(promise: nil)
            }
        }
        response.whenComplete { _ in
